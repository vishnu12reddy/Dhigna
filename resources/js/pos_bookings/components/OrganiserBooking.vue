...
<div>
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="form-group">
                    <label>{{ trans('em.search') }} {{ trans('em.bookings') }}</label>
                    <input type="text" class="form-control"  name="order_number" v-model="order_number" :placeholder="trans('em.order')+' '+trans('em.id')">
                </div>
            </div>    
            <!-- CUSTOM --> 
        </div>
    ...
    <div class="container-fluid mt-5">
                ...
                <table class="table table-hover table-striped">
                    ...
                    <tbody>
                        <tr v-for="(booking, index) in bookings" :key="index" >
                            ...
                            <td :data-title="trans('em.booked_on')">{{ moment(convert_date_to_local(booking.created_at)).format(date_format.vue_date_format) }}</td>
                            ...
                            <td :data-title="trans('em.download')">
                                <div v-if="hide_ticket_download == null">
                                    <a v-if="booking.is_paid == 1 && booking.status == 1" class="lgx-btn lgx-btn-sm lgx-btn-success" :href="downloadURL(booking.id, booking.order_number)" target="_blank" ><i class="fas fa-download"></i> {{trans('em.ticket')}}</a>
                                    <!-- CUSTOM -->
                                    <a v-if="booking.is_paid == 1 && booking.status == 1" class="lgx-btn lgx-btn-sm lgx-btn-success" :href="downloadInvoice(booking.id)" target="_blank" ><i class="fas fa-download"></i> {{trans('em.invoice')}}</a>
                                    <!-- CUSTOM -->
                            ...
                            <td :data-title="trans('em.actions')">
                                ...
                                <edit-booking 
                                    ...
                                    @changeItem = "getPosOrganiserBookings"
        ...
        <div class="row">
               <div class="col-md-12 text-center" v-if="bookings.length > 0">
                <pagination-component v-if="pagination.last_page > 1" :pagination="pagination" :offset="pagination.total" :path="'/pos-bookings/index'" @paginate="getPosOrganiserBookings()">
...
export default {
    ...
    methods:{
        ...
        getPosOrganiserBookings() {
            ...
            axios.get(route('pos.bookings')+'?page='+this.current_page+'&event_id='+this.event_id+'&start_date='
                         +this.start_date+'&end_date='+this.end_date+'&order_number='+this.order_number)
            .then(res => {
