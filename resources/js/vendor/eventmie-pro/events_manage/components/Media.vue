                    ...
                    <form ref="form" @submit.prevent="cropThumbnailPoster" method="POST" enctype="multipart/form-data" class="lgx-contactform form-horizontal">
                        ...
                        <div class="form-group" >
                           <label class="col-sm-2 control-label">YouTube {{ trans('em.video_url') }} ({{ trans('em.optional') }})</label>
                            <div class="col-md-6">
                               <button type="button" class="lgx-btn lgx-btn-sm" @click="addMore += 1">
                                   <i class="fas fa-plus-circle" aria-hidden="true">
                                        {{ trans('em.add_more') }}
                                   </i>
                                </button>

                                <button type="button" class="lgx-btn lgx-btn-sm lgx-btn-danger" @click="addMore += -1" v-if="addMore > 1">
                                ...
                                <input v-for=" (itm, ind) in addMore" :key="ind" type="text" class="form-control" name="video_link[]" placeholder="e.g Zjq1zRWpcgs"  v-model="video_link[ind]" @change="isDirty()">
    ...
    methods: {
        ...
        editMedia(){
            ...
            if(Object.keys(this.event).length > 0)
            {
                ...
                this.video_link                = JSON.parse(this.event.video_link) ==  null ? [] : JSON.parse(this.event.video_link);
                this.multiple_images           = this.event.images ? JSON.parse(this.event.images) : [];

                //CUSTOM
                this.addMore  = this.video_link.length == 0 ? 1 : this.video_link.length;
                //CUSTOM
            }    
        },
